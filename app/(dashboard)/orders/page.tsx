...
import { DataTable } from "@/components/custom ui/DataTable"
import Loader from "@/components/custom ui/Loader"
import { columns, OrderColumnType } from "@/components/orders/OrderColumns" // Import the interface
import { useEffect, useState } from "react"
import { Package, RefreshCw, Filter, ArrowDownUp } from "lucide-react"
import { Card, CardContent, CardHeader, CardTitle, CardFooter } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto bg-slate-50 min-h-screen">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Quản lý đơn hàng và theo dõi kho hàng</p>
      </div>
        ...
        <Card className="border border-slate-200 shadow-md overflow-hidden bg-white">
          <CardHeader className="bg-gradient-to-r from-indigo-500/10 to-purple-500/5 border-b border-slate-200 pb-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <CardTitle className="text-2xl font-bold text-slate-800">Đơn hàng</CardTitle>
                  <p className="text-slate-500 text-sm mt-1">Quản lý đơn hàng khách hàng và chi tiết vận chuyển</p>
                </div>
              </div>
          ...
          <div className="bg-slate-50 px-6 py-3 border-b border-slate-200">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-2 text-sm text-slate-600">
                <Filter className="h-4 w-4" />
                <span>Lọc và quản lý danh sách đơn hàng</span>
              </div>
              <div className="flex items-center space-x-2">
                <Button 
                  variant="outline" 
                ...
                >
                  <ArrowDownUp className="h-3 w-3 mr-1" />
                  Sắp xếp theo giá {sortDirection === "asc" ? "thấp nhất" : "cao nhất"}
                </Button>
              </div>
            </div>
          </div>
